...
netbsd32_syscall(struct trapframe *frame)
{
	...
	u_int nargs, code;
	register_t rval[2];
	register_t *args, copyargs[SYS_MAXSYSARGS];

	l = curlwp;
	p = l->l_proc;

	code = frame->tf_esr & (SYS_NSYSENT - 1); /* XXXARCH64 decode SWI ??? */
	...
	if (nargs > 4) {
		args = copyargs;
		memcpy(args, &tf->tf_reg[0], 4 * sizeof(register_t));
	...
	case ERESTART:
		/*
		 * Reconstruct the prc to point at the swi.
		 */
		tf->tf_pc -= 4;
		break;
